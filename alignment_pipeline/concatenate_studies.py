...
Usage:
    python concatenate_studies.py [--input-dir data/results] [--output master_expression_data.csv] [--exclude-dirs examine,example]
"""
...
def find_study_files(input_dir: str, exclude_dirs: List[str] = None) -> List[Path]:
    ...
    Args:
        ...
        exclude_dirs: List of directory names to exclude (e.g., 'examine', 'example')
    ...
    logger.info(f"Found {len(csv_files)} total CSV files, {len(filtered_files)} after filtering")
    
    if exclude_dirs:
        excluded_count = len(csv_files) - len(filtered_files)
        logger.info(f"Excluded {excluded_count} files from directories: {', '.join(exclude_dirs)}")
    
    return filtered_files

def load_and_transpose_study(csv_file: Path) -> Optional[pd.DataFrame]:
    """
    ...
    try:
        ...
        # Load the CSV file
        df = pd.read_csv(csv_file, index_col=0)  # Use first column (symbol) as index
        ...
        logger.info(f"  Loaded {study_id}: {transposed_df.shape[0]} samples, {transposed_df.shape[1]-1} genes")
...
def concatenate_studies(study_dataframes: List[pd.DataFrame]) -> pd.DataFrame:
    ...
    if duplicate_gsms.any():
        ...
        logger.warning(f"Duplicated GSMs: {list(duplicated_gsms)[:10]}{'...' if len(duplicated_gsms) > 10 else ''}")
    ...
    logger.info(f"Master dataset: {master_df.shape[0]} samples, {master_df.shape[1]-1} genes")
...
def generate_summary_report(master_df: pd.DataFrame, study_files: List[Path]) -> str:
    ...
    report += f"""
        ...
        Duplicate GSM samples: {'None' if not master_df.index.duplicated().any() else 'Found (handled)'}
...
def save_master_file(master_df: pd.DataFrame, output_file: str, compression: str = None):
    """
...
def main():
    ...
    parser.add_argument(
        ...
        help='Comma-separated list of directory names to exclude (default: examine,example)'
    )
    ...
    try:
        ...
        if args.dry_run:
            logger.info("Dry run complete. Use without --dry-run to actually concatenate.")
            return 0
        ...
        logger.info(f"Final dataset: {master_df.shape[0]:,} samples × {master_df.shape[1]-1:,} genes")
