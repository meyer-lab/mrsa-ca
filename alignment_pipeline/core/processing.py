...
from utils.file_utils import save_csv_result_securely, cleanup_temp_files
...
def download_srr(srr, input_dir, max_retries=3):
    ...
    if os.path.exists(single_path) or (os.path.exists(paired_path1) and os.path.exists(paired_path2)):
    ...
    try:
        ...
        cmd = [
            ...
            "--threads", str(download_threads),  # thread count
            "--skip-technical",  # skip technical reads
            "-O", input_dir,  # output directory
            "--temp", input_dir  # temp directory to scratch (CRITICAL for preventing home dir overflow)
        ]
        
        for attempt in range(max_retries):
            try:
                logger.info(f"Download attempt {attempt + 1}/{max_retries} for {srr} with {download_threads} threads")
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(f"Download attempt {attempt + 1}/{max_retries} failed for {srr}: {result.stderr}")
                ...
                logger.error(f"Download attempt {attempt + 1}/{max_retries} timeout for {srr}")
                if attempt < max_retries - 1:
                    import time
                    logger.info("Waiting 10s before retry after timeout...")
                    time.sleep(10)
            except Exception as e:
                logger.error(f"Download attempt {attempt + 1}/{max_retries} error for {srr}: {e}")
                if attempt < max_retries - 1:
                    import time
...
def align_srr(srr, input_dir, output_dir, genome="homo_sapiens", return_type="gene", 
              identifier="symbol", max_retries=3, cleanup_after=False):
        ...
        try:
            ...
            if not result_df.empty:
                ...
                if actual_gene_count == expected_gene_count:
                    ...
                    logger.warning(f"🚨 Existing {srr} has wrong gene count: {actual_gene_count} != {expected_gene_count}")
                    logger.warning(f"Will delete and re-align {srr}")
                    os.remove(result_path)
        except Exception as e:
            logger.warning(f"Existing result file corrupt for {srr}, will re-align: {e}")
        ...
        try:
            ...
            if read_type == 'single':
                result = xalign.align_fastq(
                    genome, file_paths[0],  # Note: xalign expects (species, fastq) order
                    aligner="kallisto",
                    t=alignment_threads,
                    verbose=True
                )
                ...
                result = xalign.align_fastq(
                    genome, [file_paths[0], file_paths[1]],  # Note: xalign expects (species, fastq) order
                    aligner="kallisto", 
                    t=alignment_threads,
                    verbose=True
                )
            ...
            if result is not None and not result.empty:
                ...
                result_df = process_alignment_result(result, srr, return_type, identifier)
            ...
            logger.warning(f"Alignment attempt {attempt + 1}/{max_retries} failed for {srr} - empty or null result")
            if attempt < max_retries - 1:
                import time
            ...
            logger.error(f"Alignment attempt {attempt + 1}/{max_retries} failed for {srr}: {e}")
            if attempt < max_retries - 1:
                import time
                wait_time = 10 * (attempt + 1)  # Progressive backoff: 10s, 20s, 30s
                logger.info(f"Waiting {wait_time}s before retry...")
        ...
        if isinstance(result, pd.DataFrame):
            if return_type == "gene":
                ...
                try:
                    ...
                    gene_result = xalign.ensembl.agg_gene_counts(result, "homo_sapiens", identifier=identifier)
                    # gene_result should have columns like ['gene', 'counts']
                    if hasattr(gene_result, 'iloc') and len(gene_result) > 0:
                        gene_result_df = pd.DataFrame(gene_result['counts'].values, 
                    ...
                    logger.warning(f"Gene aggregation failed for {srr}, using transcript counts: {e}")
                    # Fall back to transcript counts
                    result_df = pd.DataFrame(result['reads'].values, 
...
def combine_srr_results(srr_results, sample_id, combination_method="sum"):
    ...
    if not valid_results:
        ...
        raise ValueError(f"No valid results for sample {sample_id} - pipeline cannot proceed")
    ...
    try:
        ...
        for i, df in enumerate(valid_results):
            ...
            if len(numeric_cols) > 0:
                ...
                try:
                    ...
                    logger.error(f"❌ CRITICAL: Cannot convert data to integer for sample {sample_id}, result {i}: {ve}")
                    problematic_indices.append(i)
            else:
                logger.error(f"❌ CRITICAL: No numeric columns found in result {i} for sample {sample_id}")
                problematic_indices.append(i)
        
        if problematic_indices:
            raise ValueError(f"Data type errors in sample {sample_id} at result indices: {problematic_indices}")
        
        if not cleaned_results:
            logger.error(f"❌ CRITICAL: No valid numeric data found for sample {sample_id}")
        ...
        if len(cleaned_results) != len(srr_results):
            ...
            logger.error(f"❌ CRITICAL: Missing {missing_count} SRR results for sample {sample_id}")
            raise ValueError(f"Incomplete data for sample {sample_id}: expected {len(srr_results)}, got {len(cleaned_results)}")
        ...
        if combination_method == "sum":
            ...
            logger.error(f"❌ CRITICAL: Unknown combination method: {combination_method}")
        ...
        logger.info(f"✅ STRICT SUCCESS: Combined {len(cleaned_results)} SRRs for sample {sample_id}")
        return sample_df
        
    except Exception as e:
        logger.error(f"❌ CRITICAL FAILURE: Cannot combine results for sample {sample_id}: {e}")
