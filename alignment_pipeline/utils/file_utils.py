...
def save_csv_result_securely(result_df, result_path, max_retries=3):
    ...
    if actual_gene_count != expected_gene_count:
        logger.error(f"❌ GENE COUNT VALIDATION FAILED for {os.path.basename(result_path)}")
        ...
        logger.error(f"🚨 REFUSING TO SAVE CORRUPTED DATA - will regenerate alignment")
        return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if len(verification_df) != expected_gene_count:
                raise ValueError(f"Gene count mismatch after write: {len(verification_df)} != {expected_gene_count}")
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(f"Failed to save {result_path} after {max_retries} attempts")
...
def create_intermediate_backup(output_dir, study_id=None):
    ...
    try:
        ...
        logger.info(f"Created backup of {len(result_files)} result files in {backup_dir}")
        return True
...
def check_disk_space(directory, min_gb=10):
    ...
    try:
        ...
        if free_gb < min_gb:
            logger.warning(f"Low disk space: only {free_gb} GB available in {directory}")
            return False
        return True
    except Exception as e:
