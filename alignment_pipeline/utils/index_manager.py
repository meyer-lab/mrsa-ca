    ...
    try:
        ...
        if aligner == "kallisto":
            expected_index_path = os.path.join(
                data_path, "index", str(release), f"kallisto_{species}.idx"
            )
            ...
            if not os.path.exists(index_path):
                ...
                if allow_build:
                    if verbose:
                        logger.info(
                            f"Attempting to build kallisto index for {species} (release {release})"
                        )
                    try:
                        ...
                        if os.path.exists(expected_index_path):
                            ...
                            if os.path.exists(index_dir):
                                ...
                                if idx_files:
                                    ...
                                    if verbose:
                                        logger.info(
                                            f"Found alternative index file: {index_path}"
                                        )
                                else:
                                    logger.error(
                                        f"No .idx files found after build attempt in {index_dir}"
                                    )
                                    return False
                            else:
                                logger.error(
                                    f"Index directory not created: {index_dir}"
                                )
                                return False
    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    f"Failed to remove {dir_path} after {max_retries} attempts"
                )
...
def validate_and_reset_if_needed(
    species="homo_sapiens", aligner="kallisto", release=None, force_reset=False
):
    """
    ...
    """
    if force_reset:
        return reset_alignment_indexes(
            species=species, aligner=aligner, release=release, force=True
        )
