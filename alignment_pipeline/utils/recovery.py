        ...
        for filename in os.listdir(output_dir):
            if filename.endswith('_result.csv') and not filename.startswith('GSM'):
                ...
                try:
                    ...
                    logger.warning(f"⚠️ Invalid result file for {srr_id}, will reprocess")
                    try:
...
def check_missing_samples(study_id, sample_to_srrs, output_dir):
    ...
    try:
        ...
        samples_to_process = {sample_id: sample_to_srrs[sample_id] 
...
def validate_srr_results(sample_to_srrs, output_dir, missing_action="error"):
    ...
    if all_present:
        ...
        logger.warning(f"⚠️ Missing {len(missing_srrs)} out of {total_expected} SRR files")
...
def recover_missing_srrs(missing_srrs, input_dir, output_dir, genome="homo_sapiens", 
                        return_type="gene", identifier="symbol", max_retries_per_srr=3):
    """
    ...
    logger.info(f"🔄 STRICT RECOVERY: Attempting to recover {len(missing_srrs)} missing SRRs")
    logger.warning("⚠️  ZERO TOLERANCE MODE: ALL SRRs must be recovered or pipeline will fail")
    
    # Import core processing functions (avoid circular imports)
    from core.processing import download_srr, align_srr
    ...
    for srr in missing_srrs:
        ...
        for attempt in range(max_retries_per_srr):
            try:
                logger.info(f"🔄 Recovery attempt {attempt + 1}/{max_retries_per_srr} for {srr}")
                ...
                result = align_srr(srr, input_dir, output_dir, genome, return_type, 
                                 identifier, cleanup_after=True)
                
                if result is not None and not result.empty:
                    recovered_count += 1
                    logger.info(f"✅ STRICT RECOVERY SUCCESS: {srr} recovered successfully")
                ...
                last_error = str(e)
                logger.warning(f"⚠️  Recovery attempt {attempt + 1} failed for {srr}: {e}")
                
                if attempt < max_retries_per_srr - 1:
                    logger.info(f"🔄 Retrying {srr} (attempt {attempt + 2}/{max_retries_per_srr})")
                    # Add small delay between retries
                    import time
                    time.sleep(5)
        
        if not recovery_success:
            logger.error(f"❌ CRITICAL: Failed to recover {srr} after {max_retries_per_srr} attempts")
            logger.error(f"   Last error: {last_error}")
            unrecoverable_srrs.append(srr)
    
    # STRICT EVALUATION
    total_missing = len(missing_srrs)
    logger.info(f"🔄 STRICT RECOVERY COMPLETE: {recovered_count}/{total_missing} SRRs recovered")
    
    if unrecoverable_srrs:
        logger.error(f"❌ CRITICAL FAILURE: {len(unrecoverable_srrs)} SRRs could not be recovered")
        ...
        raise ValueError(f"Recovery failed for {len(unrecoverable_srrs)} SRRs: {sorted(unrecoverable_srrs)}")
