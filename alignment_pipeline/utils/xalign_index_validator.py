...
def check_index_integrity(data_path: str) -> Dict[str, any]:
    ...
    try:
        if not os.path.exists(data_path):
            ...
            integrity_info['recommendations'].append(f"Create data directory: {data_path}")
        ...
        for root, dirs, files in os.walk(data_path):
            for file in files:
                ...
                if any(file.endswith(ext.replace('*', '')) for ext in ['.idx', '.fa', '.fasta', '.gtf', '.gff']):
                    found_files.append({
                        'path': file_path,
        ...
        if len(found_files) == 0:
            ...
            integrity_info['recommendations'].append("No reference indexes found - will be built on first use")
        elif integrity_info['total_size_gb'] < 0.1:
            integrity_info['status'] = 'suspicious_size'
            integrity_info['recommendations'].append("Index files seem unusually small - may be corrupted")
        else:
            integrity_info['status'] = 'indexes_present'
            integrity_info['recommendations'].append("Reference indexes found and appear normal")
        ...
        for file_info in found_files:
            if file_info['size_mb'] == 0:
                ...
                integrity_info['recommendations'].append(f"Remove zero-byte file: {file_info['path']}")
    ...
    try:
        ...
        try:
            ...
            if 'homo_sapiens' in organisms:
                ...
                test_results['error_messages'].append("Ensembl connection successful but 'homo_sapiens' not found")
        except Exception as e:
...
def generate_index_report(data_path: Optional[str] = None) -> str:
    """
    ...
    report_lines.append(f"   Data Path: {'✅' if func_test['data_path_accessible'] else '❌'}")
    report_lines.append(f"   Ensembl: {'✅' if func_test['ensembl_connection'] else '⚠️'}")
    ...
    if 'error' not in space_info:
        ...
        report_lines.append(f"   Status: {'✅ Sufficient' if space_sufficient else '⚠️ Limited'}")
    else:
    ...
    if integrity['indexes_found']:
        ...
        for idx in integrity['indexes_found'][:5]:  # Show first 5
            report_lines.append(f"     - {os.path.basename(idx['path'])} ({idx['size_mb']:.1f}MB)")
        if len(integrity['indexes_found']) > 5:
            report_lines.append(f"     ... and {len(integrity['indexes_found']) - 5} more")
...
def clean_corrupted_indexes(data_path: Optional[str] = None, dry_run: bool = True) -> Dict[str, any]:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="XAlign index validation and monitoring")
    parser.add_argument("--report", action="store_true", help="Generate comprehensive report")
    parser.add_argument("--clean", action="store_true", help="Clean corrupted indexes")
    parser.add_argument("--dry-run", action="store_true", help="Dry run for cleaning (default)")
    ...
    if args.clean:
        ...
        if dry_run and cleanup['files_to_remove']:
            print("\nThis was a dry run. Use --clean without --dry-run to actually remove files.")
