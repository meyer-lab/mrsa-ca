...
wandb_%: .venv
	...
	mkdir -p ./wandb_logs
	PYTHONPATH=. rye run python -c "from mrsa_ca_rna.wb.wandb_$* import perform_wandb_experiment; perform_wandb_experiment()" > ./sweep_logs/$*_$(shell date +%Y%m%d%_H%M%S).log 2>&1
