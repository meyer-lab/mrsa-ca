...
def figure12a_setup(rank=20):
    ...
    melted["Rank"] = (
        melted["Rank"].astype(str) + "_" + melted["Disease"].replace(mapping)
    )

    x_data = melted.loc[:, ["Factor Value"]]
    y_data = melted.loc[:, "Disease"]

    # perform logistic regression on the melted dataframe
    score, model = perform_PC_LR(x_data, y_data, return_clf=True)
...
def genFig():
    ...
    for i, rank in enumerate([20, 50]):
        ...
        y_pred = cross_val_predict(
            model, X=melted.loc[:, ["Factor Value"]], y=melted["Disease"], cv=5
        )

        conf_matrix = confusion_matrix(
            melted["Disease"],
            y_pred,
            labels=["MRSA", "Candidemia", "Healthy", "BreastCancer"],
        )
        ...
        pred_labels = [
            "Predicted MRSA",
            "Predicted Candidemia",
            "Predicted Healthy",
            "Predicted BreastCancer",
        ]
