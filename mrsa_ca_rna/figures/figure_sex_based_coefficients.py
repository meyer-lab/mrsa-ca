"""This file explores sex-based determinants of MRSA data through gene coefficient analysis."""
...
def get_gene_tiers():
    ...
    tier_1 = [
        "GLS2", "HLA-DRA", "CD74", "DNMT3A", "IL10", "STAT1", "CEBPB", 
        "TIRAP", "IL1RN", "IL27", "CHI3L1", "CCL2", "ANGPT2", "CCL26", 
        "CSCL8", "SELE", "IL2RA", "LCN2", "IL1B", "IL6", "CXCL1",
    ]
    
    # Tier 2 genes
    tier_2 = tier_1 + [
        "TNF", "IFNG", "IL4", "IL13", "CCR2", "TLR2", "TLR4"
    ]
    
    # Tier 3 genes
    tier_3 = tier_2 + [
        "ICAM1", "VCAM1", "MMP9", "TIMP1", "FOXP3", "CD4", "CD8A"
    ]
    
    # Tier 4 genes
    tier_4 = tier_3 + [
        "IL17A", "IL22", "IL23A", "RORC", "TGFB1", "SMAD3", "SMAD7"
    ]
...
def plot_coefficients_for_tier(ax, data, gene_list, tier_name, top_n=10):
    ...
    top_genes = (coeffs.abs().mean(axis=1)
                 ...
                 .head(top_n)  # Show only top N most predictive genes
                 .index.tolist())
    ...
    ax.set_title(f"Top {top_n} Predictive Genes - {tier_name} ({len(X.columns)} genes)"
