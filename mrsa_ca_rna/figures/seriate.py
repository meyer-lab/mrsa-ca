...
ortools8_10 = Version("8.0.0") <= ortools_version < Version("10")
if not ortools8_10:
    raise ImportError(
        "No valid version of ortools installed. Please install ortools 8 or 9."
    )
...
def seriate(
    ...
    if timeout > 0:
        return _seriate(
            dists=dists,
            approximation_multiplier=approximation_multiplier,
            timeout=timeout,
        )
    elif timeout < 0:
...
def _seriate(
    ...
    if squareform:
        ...
        assert (
            int(numpy.round(numpy.sqrt(1 + 8 * dists.shape[0]))) ** 2
            == 1 + 8 * dists.shape[0]
        )
    ...
    if assignment is None:
        raise IncompleteSolutionError(
            "No solution was found. Please increase the timeout value or set it to 0."
        )
