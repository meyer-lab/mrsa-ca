#!/usr/bin/env python3
"""
Process SRR accession numbers in parallel by:
1. Downloading all SRRs in parallel
2. Aligning each SRR as it becomes available
3. Combining all results at the end

With improved error handling and file synchronization.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('process_srr.log')
    ]
)
        ...
        if os.path.exists(file_path):
            try:
                ...
                with open(file_path, 'rb') as f:
                    # Actually read a little more data to ensure file is accessible
                    f.read(1024)
                ...
                logger.info(f"File {file_path} exists but is not ready yet: {str(e)}. Waiting...")
        else:
    ...
    if os.path.exists(fastq_path):
        try:
            with open(fastq_path, 'rb') as f:
                f.read(1024)  # Read more data to ensure file is valid
            logger.info(f"FASTQ for {srr} already exists and is readable, skipping download.")
            return srr, True
        except IOError:
            logger.warning(f"FASTQ for {srr} exists but is not readable. Will attempt to redownload.")
        ...
        for retry in range(max_retries):
            try:
                ...
                if time.time() - start_time > max_download_time:
                    logger.error(f"Download of {srr} timed out after {max_download_time} seconds")
                    continue
                
                # Wait for file to become available
                if wait_for_file(fastq_path, timeout=1800, check_interval=60, post_detection_wait=10):
...
def align_srr(srr, input_dir, output_dir, genome="human", return_type="gene", identifier="symbol", max_retries=3, cleanup_after=False):
    ...
    if os.path.exists(result_path):
        try:
            ...
            logger.info(f"Result for {srr} already exists and is readable, skipping alignment.")
            
            # Cleanup if requested
            if cleanup_after and os.path.exists(fastq_path):
                try:
                    os.remove(fastq_path)
                    logger.info(f"Deleted FASTQ file {fastq_path} after successful alignment")
                except Exception as e:
                    logger.warning(f"Failed to delete FASTQ file {fastq_path}: {str(e)}")
            
            return result_df
        except Exception as e:
            logger.warning(f"Error reading existing result for {srr}: {str(e)}. Will realign.")
    ...
    for retry in range(max_retries):
        try:
            ...
            result = a4.align.fastq(genome, fastq_path, return_type=return_type, identifier=identifier)
            ...
            if isinstance(result, pd.Series):
                ...
                logger.error(f"Error: Alignment for {srr} returned unexpected type: {type(result)}")
                continue  # Try again
            ...
            for save_retry in range(3):
                try:
                    ...
                    logger.warning(f"Error saving result for {srr} (attempt {save_retry+1}): {str(e)}")
                    time.sleep(2)
...
def process_srrs_pipeline(srr_list, input_dir, output_dir, max_workers=4, genome="human", return_type="gene", 
                          identifier="symbol", cleanup="end"):
    """
    ...
    # Create two single-worker executors
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as download_executor, \
        ...
        if ready_for_alignment:
            ...
            active_align_future = align_executor.submit(align_srr, srr, input_dir, output_dir, 
                                                        genome, return_type, identifier, max_retries=3, cleanup_after=(cleanup == "immediate"))
        ...
        while active_download_future is not None or active_align_future is not None or \
            ...
            if active_align_future is None and ready_for_alignment and current_align_srr is None:
                srr = ready_for_alignment.pop(0)
                active_align_future = align_executor.submit(align_srr, srr, input_dir, output_dir, 
                                                          genome, return_type, identifier, max_retries=3, cleanup_after=(cleanup == "immediate"))
                current_align_srr = srr
    ...
    # Add this block in the process_srrs_pipeline function before the final concatenation
    # Try to combine all results
    if all_results:
        try:
            logger.info(f"Combining results from {len(all_results)} successful alignments")
            ...
            for result in all_results:
                if isinstance(result, pd.Series):
                    ...
                    srr_id = result.name if hasattr(result, 'name') and result.name is not None else 'unknown'
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Process SRR accessions in parallel")
    parser.add_argument("--srr_file", required=True, help="Path to file with SRR accessions (one per line)")
    parser.add_argument("--input_dir", default="data/input", help="Directory for downloaded FASTQ files")
    parser.add_argument("--output_dir", default="data/output", help="Directory for output files")
    ...
    parser.add_argument("--return_type", default="gene", help="Return type for alignment")
    parser.add_argument("--identifier", default="symbol", help="Identifier type for alignment")
    parser.add_argument("--batch_mode", action="store_true", help="Run in batch mode with periodic saves")
    parser.add_argument("--cleanup", choices=["none", "immediate", "end"], default="end", help="Cleanup strategy for FASTQ files")
    parser.add_argument("--reset-archs4", action="store_true", 
                        help="Reset ARCHS4 data files before processing")
    
    args = parser.parse_args()
    
    # Reset ARCHS4 if requested
    if args.reset_archs4:
        import shutil
        import archs4py
        archs4_path = os.path.dirname(archs4py.__file__)
