...
def test_prepare_data_raw_layer_and_normalization(sample_adata):
    ...
    assert np.all(adata.layers["raw"] >= 0), \
        "Raw layer contains negative values"
...
@pytest.mark.parametrize("filter_threshold,min_pct", [
...
def test_prepare_data_disease_labels(sample_adata):
    ...
    expected_diseases = {
        "MRSA", "CANDIDA", "BREAST_CANCER", "UC_PSC", "TB", "T1DM", 
        "COVID", "LUPUS", "HIV_CM", "ENTEROVIRUS", "ZIKA", "HEALTHY",
        "RA", "HBV", "KIDNEY", "COVID_MARINES", "BREAST_CANCER_TCR",
        "SEPSIS", "LEUKEMIA", "COVID_SINAI", "ASTHEMA", "AETHERSCLEROSIS"
    }
...
def test_prepare_data_reproducibility():
    ...
    assert np.allclose(adata1.X, adata2.X), "Inconsistent main matrix between runs"
    assert np.allclose(adata1.layers["raw"], adata2.layers["raw"]), \
...
def test_load_expression_data_basic(raw_adata):
    ...
    assert adata.n_obs > 0, "Dataset has no observations"
    assert adata.n_vars > 0, "Dataset has no variables"
    assert not np.any(np.isnan(adata.X)), "Raw data contains NaN values"
